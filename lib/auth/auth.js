...
export default class Auth {
  ...
  watchTokenExpiration () {
    const { $axios } = this.ctx.app
    let isRefreshing = false

    $axios.onRequest(async (config) => {
      let originalRequest = config
      let { token, refreshToken } = this

      // token or refreshToken does not exists
      if (!token || !token.length || !refreshToken || !refreshToken.length) {
        return originalRequest
      }

      // if trying to refesh token, do not try again
      if (!isRefreshing) {
        ...
        if (now > token_expires_at) {
          ...
          if (now < refresh_token_expires_at) {
            ...
            return $axios.post(this.strategy.options.access_token_endpoint,
              ...
              token = this.strategy.options.token_type + ' ' + response.data.access_token
              refreshToken = this.strategy.options.token_type + ' ' + response.data.refresh_token
