...
import { encodeQuery, logger } from './utilities'
...
export default class Auth {
  ...
  mounted () {
    ...
    // handle token expiration, if axios credentials is eneabled
  ...
  get token () {
    return this.getToken(this.$state.strategy)
  }
  ...
  syncToken (strategy) {
    ...
    const token = this.$storage.syncUniversal(_key)
    this._setToken(token)
  ...
  _setToken (token) {
    ...
    this.ctx.app.$axios.setToken(token)
  }

  _clearToken () {
    // Clear Authorization token for all axios requests
    this.ctx.app.$axios.setToken(false)
  }
  ...
  get refreshToken () {
    return this.getRefreshToken(this.$state.strategy)
  }

  getRefreshToken (strategy) {
    const _key = this.options.refreshToken.prefix + strategy
  ...
  setUser (user) {
    // handle resource access for oauth2
    if (user && this.strategy.options.decode_resource_access === true) {
      user.resource_access = jwtDecode(this.token).resource_access
    }
  ...
  // ---------------------------------------------------------------
  // Watch axios requests 
  // ---------------------------------------------------------------

  watchTokenExpiration () {
    let $axios = this.ctx.app.$axios
    ...
    $axios.onRequest(async (config) => {
      ...
      // token and refreshToken exists
      if (!isRefreshing && token && token.length && refreshToken && refreshToken.length) {
        const token_expires_at = jwtDecode(token).exp * 1000
        const refresh_token_expires_at = jwtDecode(refreshToken).exp * 1000
        const now = Date.now()

        // token has expired
        if (now > token_expires_at) {
          logger('hold original request, because token has expired.')
          ...
          if (now < refresh_token_expires_at) {
            ...
            return $axios.post(this.strategy.options.access_token_endpoint,
              encodeQuery({
              ...
              const newToken = this.strategy.options.token_type + ' ' + response.data.access_token
              const newRefreshToken = this.strategy.options.token_type + ' ' + response.data.refresh_token
              this.setToken(this.strategy.name, newToken)
              this.setRefreshToken(this.strategy.name, newRefreshToken)
              logger('* token has been refreshed *')

              originalRequest.headers['Authorization'] = newToken

              logger('Making original request to: ' + originalRequest.url)
            ...
            logger('could not refresh it because refresh_token also has expired. Logout now ...')
            this.logout()
          }
        }
      }
