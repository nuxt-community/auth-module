...
export default class Auth {
  ...
  _setToken (token) {
    ...
    this.ctx.app.$axios.setToken(token, this.strategy.options.token_type)
  }
  ...
  watchTokenExpiration () {
    ...
    $axios.onRequest(async (config) => {
      ...
      if (!isRefreshing && token && token.length && refreshToken && refreshToken.length) {
        ...
        const token_expires_at = decodedToken.exp * 1000
        const refresh_token_expires_at = decodedRefreshToken.exp * 1000
        const now = Date.now()

        // token has expired
        if (now > token_expires_at) {
          console.log('token has expired, trying refresh it')
          ...
          if (now < refresh_token_expires_at) {
            ...
            console.log('refresh token also has expired. Logout ...')
            this.logout()
          }
        }
      }
