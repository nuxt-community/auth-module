...
export default class Auth {
  ...
  request (endpoint, defaults) {
    ...
    return this.ctx.app.$axios
      .request(_endpoint)
      .then(({ data }) => {
        if (typeof this.options.requestHandler === 'function') {
          this.options.requestHandler(endpoint, data)
        }

        return _endpoint.propertyName ? getProp(data, _endpoint.propertyName) : data
      })
      .catch(error => {
