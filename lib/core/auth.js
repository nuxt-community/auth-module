...
export default class Auth {
  ...
  reset () {
    if (!this.strategy.reset) {
      ...
      this.setRemember(this.$state.strategy, false)
      return Promise.resolve()
    }
  ...
  setRemember (strategy, remember) {
    const _key = this.options.remember.prefix + strategy
    const options = {}
    const rememberFor = this.strategies[this.$state.strategy].options.endpoints.login.rememberFor

    if (remember && rememberFor !== false) {
      options.expires = rememberFor
    }

    console.log('setting');

    this.$storage.setState('remember', remember)
