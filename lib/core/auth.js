...
export default class Auth {
  ...
  networkHandler (method, ...options) {
    const networkHandler = this.ctx.app[this.options.networkHandler]
    ...
    if (options.length) {
      return networkHandler[method].apply(networkHandler, options)
    }

    return networkHandler[method]
  }
  ...
  request (endpoint, defaults, withResponse) {
    ...
    if (!this.networkHandler()) {
      // eslint-disable-next-line no-console
      console.error('[AUTH] add the module of your network handler to nuxt.config file')
      return
    }

    return this.networkHandler(_endpoint.method, _endpoint.url, _endpoint.data, _endpoint.config)
      .then(async response => {
        const data = response.json ? await response.json() : response.data
        const result = _endpoint.propertyName ? getProp(data, _endpoint.propertyName) : data
  ...
  requestWith (strategy, endpoint, defaults, withResponse) {
    ...
    if (!_endpoint.config.headers) {
      _endpoint.config.headers = {}
    }
    if (!_endpoint.config.headers[tokenName] && isSet(token) && token) {
      _endpoint.config.headers[tokenName] = token
    }
