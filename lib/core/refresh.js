...
export class RefreshController {
  ...
  initializeAutomaticRefresh () {
    ...
    if (this.scheme.options.autoRefresh) {
  ...
  initializeScheduledRefresh () {
    ...
    let intervalDurationMillis = (this.scheme.tokenStatus.getTokenExpiration() - Date.now()) * 0.75
  ...
  initializeRequestInterceptor (refreshEndpoint) {
    this.$auth.ctx.app.$axios.onRequest(async config => {
      ...
      this.scheme.tokenStatus.syncTokenExpiration()
      this.scheme.tokenStatus.syncRefreshTokenExpiration()
...
export class TokenExpirationStatus {
  ...
  _calculateTokenStatus () {
    ...
    try {
      tokenExpiresAt = this.getTokenExpiration()
  ...
  getTokenExpiration () {
    const _key = this.scheme.options.tokenExpirationPrefix + this.scheme.name
  ...
  getRefreshTokenExpiration () {
    const _key = this.scheme.options.refreshTokenExpirationPrefix + this.scheme.name

    return this.$auth.$storage.getUniversal(_key)
  }

  setRefreshTokenExpiration (token) {
    const _key = this.scheme.options.refreshTokenExpirationPrefix + this.scheme.name

    return this.$auth.$storage.setUniversal(_key, token)
  }

  syncRefreshTokenExpiration () {
    const _key = this.scheme.options.refreshTokenExpirationPrefix + this.scheme.name

    return this.$auth.$storage.syncUniversal(_key)
  }

  updateTokenExpiration (result = {}) {
    const token = this.scheme.$auth.getToken(this.scheme.name)
    let decodedToken, tokenExpiration
    const _tokenIssuedAtMillis = getProp(result, this.scheme.options.issuedAt) || Date.now()
    const _tokenTTLMillis = (getProp(result, this.scheme.options.expiresIn) || this.scheme.options.token.maxAge) * 1000
    const _tokenExpiresAtMillis = getProp(result, this.scheme.options.expiresAt) || _tokenIssuedAtMillis + _tokenTTLMillis
    ...
    try {
      ...
      tokenExpiration = _tokenExpiresAtMillis || false
    ...
    this.setTokenExpiration(tokenExpiration)
  ...
  updateRefreshTokenExpiration () {
    ...
    try {
      ...
      // If the token is not jwt, we can't decode and refresh it, use _tokenExpiresAt value
      refreshTokenExpiration = _tokenExpiresAtMillis || false
