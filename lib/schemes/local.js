...
export default class LocalScheme {
  constructor (auth, options) {
    ...
    this.options.tokenProperty = this.options.endpoints.login.propertyName || this.options.tokenProperty
    this.options.userProperty = this.options.endpoints.user.propertyName || this.options.userProperty
  }
  ...
  async login (endpoint) {
    ...
    // Update Token
    if (this.options.tokenRequired) {
      let token = getProp(loginResult, this.options.tokenProperty)
  ...
  async logout (endpoint = {}) {
    ...
    if (this.options.endpoints.logout) {
      ...
      const clientId = this.options.dataClientIdProperty
      if (clientId) {
        if (!endpoint.data) {
          endpoint.data = {}
        }
