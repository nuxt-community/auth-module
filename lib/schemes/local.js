...
export default class LocalScheme {
  ...
  async _updateTokens (result) {
    ...
    if (this.options.refreshToken) {
      ...
      var tokenExpiration = tokenCreatedAt + (getProp(result, this.options.endpoints.refresh.expiresIn) * 1000)
    }
  ...
  mounted () {
    ...
    return this.$auth.fetchUserOnce().then((response) => {
      if (this.options.refreshToken) {
        this.$auth.ctx.app.$axios.interceptors.response.use(null,
          async (error) => {
            if (error.config && error.response && error.response.status === 401 && !error.config.__isRetryRequest) {
              ...
              if (headerToken !== token) {
                ...
                error.config.headers[this.options.tokenName] = this.$auth.getToken(this.name)
