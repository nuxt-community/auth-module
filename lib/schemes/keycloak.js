...
export default class KeycloakScheme {
  ...
  updateOptionsForBrowserHostname() {
    if (window && window.location && window.location.hostname) {
      ...
      const keycloak_port = this.options.keycloak_port
      const realm = this.options.realm
      const nuxt_host = window.location.hostname
      const nuxt_port = this.options.nuxt_port
      const logout_referrer = this.options.logout_referrer
      const client_id = this.options.client_id

      const keycloak_host = `${window.location.hostname}`
      const base_uri = `${protocol}://${keycloak_host}${keycloak_port}/auth/realms/${realm}/protocol/openid-connect`
      const account_uri = `${protocol}://${keycloak_host}${keycloak_port}/auth/realms/${realm}/account`
      const authorization_uri = `${base_uri}/auth`
      const userinfo_uri = `${base_uri}/userinfo`
      const cert_uri = `${base_uri}/certs`
      const referrer_uri = `${protocol}://${window.location.hostname}${nuxt_port}`
      const redirect_uri = `${protocol}://${window.location.hostname}${nuxt_port}`
      const logout_redirect_uri = `${protocol}://${window.location.hostname}${nuxt_port}/logout`
      const logout_referrer_uri = `${protocol}://${window.location.hostname}${nuxt_port}`
