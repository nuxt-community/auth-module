...
export default class KeycloakScheme {
  constructor (auth, userOptions) {
    ...
    let base_uri = userOptions.base_uri || `${protocol}://${keycloak_host}/auth/realms/${realm}/protocol/openid-connect`
    ...
    let use_browser_host = userOptions.hasOwnProperty('use_browser_host') ? userOptions.use_browser_host : true;
    ...
    // @ts-ignore
    if (use_browser_host && process.browser && window.location.hostname) {
      nuxt_host = window.location.hostname
      keycloak_host = `${window.location.hostname}`
      base_uri = `${protocol}://${keycloak_host}${keycloak_port}/auth/realms/${realm}/protocol/openid-connect`
      account_uri = `${protocol}://${keycloak_host}${keycloak_port}/auth/realms/${realm}/account`
      authorization_uri = `${base_uri}/auth`
      userinfo_uri = `${base_uri}/userinfo`
      cert_uri = `${base_uri}/certs`
      referrer_uri = `${protocol}://${window.location.hostname}${nuxt_port}`
      redirect_uri = `${protocol}://${window.location.hostname}${nuxt_port}`
      logout_redirect_uri = `${protocol}://${window.location.hostname}${nuxt_port}/logout`
      logout_referrer_uri = `${protocol}://${window.location.hostname}${nuxt_port}`
      logout_uri = `${base_uri}/logout?` + encodeQuery({ referrer: logout_referrer, referrer_uri: logout_referrer_uri, redirect_uri: logout_redirect_uri })
      access_token_uri = userOptions.access_token_uri || `${base_uri}/token`

      this.$auth.$storage.setUniversal('app_host_uri', `${protocol}://${nuxt_host}${nuxt_port}/`)
      this.$auth.$storage.setUniversal('base_uri', base_uri)
      this.$auth.$storage.setUniversal('account_uri', account_uri)
      this.$auth.$storage.setUniversal('client_id', client_id)
    }

    // syncUniversal won't set the value unless it the storage doesn't currently have a value
    this.$auth.$storage.syncUniversal('app_host_uri', `${protocol}://${nuxt_host}${nuxt_port}/`)
    this.$auth.$storage.syncUniversal('base_uri', base_uri)
    this.$auth.$storage.syncUniversal('account_uri', account_uri)
    this.$auth.$storage.syncUniversal('client_id', client_id)
