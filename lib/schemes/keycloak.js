...
export default class KeycloakScheme {
  constructor(auth, userOptions) {
    ...
    let keycloak_ssl_port = userOptions.keycloak_ssl_port ? `:${userOptions.keycloak_ssl_port}` : ':8181'
  ...
  updateOptionsForBrowserHostname() {
    if (window && window.location && window.location.hostname) {
      ...
      const keycloak_port = isProxied ? this.options.keycloak_ssl_port : this.options.keycloak_port      
