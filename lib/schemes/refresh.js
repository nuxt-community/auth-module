...
export default class RefreshScheme extends LocalScheme {
  ...
  async _updateTokens (result) {
    if (this.options.tokenRequired) {
      let token = getProp(result, this.options.tokenProperty)
      ...
      const refreshToken = getProp(result, this.options.refreshTokenProperty)
      if (refreshToken !== undefined) {
        this._setRefreshToken(refreshToken)

        const _tokenCreatedAt = getProp(result, this.options.createdAtProperty) || Date.now()
        const _tokenTTL = getProp(result, this.options.expiresInProperty) || this.options.defaultExpirationTime
        const tokenExpiration = _tokenCreatedAt + (_tokenTTL * 1000)
        this._setExpiration(tokenExpiration)
      }

      // Update client id
      const clientId = getProp(result, this.options.clientIdProperty)
      if (clientId) {
  ...
  async _tokenRefresh () {
    ...
    let { dataClientIdProperty, dataGrantTypeProperty, grantType } = this.options
    const endpoint = {
      data: {
        [this.options.dataRefreshTokenProperty]: this._getRefreshToken()
      }
    }

    // Only add client id to payload if enabled
    if (dataClientIdProperty) {
      endpoint.data[dataClientIdProperty] = this.$auth.getClientId(this.name)
    }

    // Only add grant type to payload if enabled
    if (dataGrantTypeProperty !== false) {
      endpoint.data[dataGrantTypeProperty] = grantType
    }
  ...
  async fetchUser (endpoint) {
    ...
    if (!requestFailed) {
      user = getProp(user, this.options.userProperty)
  ...
  async logout (endpoint = {}) {
    ...
    if (this.options.endpoints.logout) {
      ...
      const refreshToken = this.options.dataRefreshTokenProperty
      if (refreshToken) {
        if (!endpoint.data) {
          endpoint.data = {}
        }
...
const DEFAULTS = {
  ...
  dataRefreshTokenProperty: 'refresh_token',
  dataClientIdProperty: 'client_id',
  dataGrantTypeProperty: 'grant_type',
