...
export default class RefreshScheme extends LocalScheme {
  ...
  async mounted () {
    if (this.options.tokenRequired) {
      ...
      const now = new Date().setMilliseconds(0)
      const tokenExpiration = new Date(this._syncTokenExpiration()).setMilliseconds(0)
      const refreshTokenExpiration = new Date(this._syncRefreshTokenExpiration()).setMilliseconds(0)

      if (now >= refreshTokenExpiration) {
        await this._logoutLocally()
      } else if (this.options.autoLogout && now >= tokenExpiration) {
        await this._logoutLocally()
      }
