...
import RefreshTokenResponseError from '../includes/RefreshTokenResponseError'
import FetchUserResponseError from '../includes/FetchUserResponseError'
...
export default class RefreshScheme extends LocalScheme {
  ...
  /*
  * This method checks if we have a `token` or a `refreshToken` to proceed
  * */
  ...
  async login (endpoint) {
    ...
    /*
    * Assign tokens based on the `property` key provided in the `auth.strategy_name.token` & `auth.strategy_name.refreshToken` config
    * auth:{
    *   strategy_name:{
    *     token:{
    *       property:'token'
    ...
    * */
    ...
    /*
    * Fetch the user details if enabled via `autoFetch`
    *
    * auth:{
    *   strategy_name:{
    *     ...
  ...
  async fetchUser (endpoint) {
    ...
    const response = await this.$auth.requestWith(
      ...
      throw new FetchUserResponseError()
    })
