...
export default class RefreshScheme extends LocalScheme {
  ...
  async _updateTokens (result) {
    ...
    var tokenCreatedAt = getProp(result, this.options.createdAtProperty) || Date.now()
    var refreshToken = getProp(result, this.options.endpoints.refresh.propertyName)
    var defaultExpirationTime = parseInt(this.options.defaultExpirationTime) || 1800
    var tokenExpiration = getProp(result, this.options.expiresInProperty)
    var clientId = getProp(result, this.options.clientIdProperty)
