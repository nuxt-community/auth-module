...
export default class RefreshScheme extends LocalScheme {
  ...
  async _tokenRefresh (self) {
    ...
    let { clientIdProperty, grantTypeProperty, grantType } = this.options
    const endpoint = {
      data: {
        [getRealName(this.options.endpoints.refresh.propertyName)]: this._getRefreshToken()
      }
    }

    // Only add client id to payload if enabled
    if (clientIdProperty) {
      endpoint.data[getRealName(clientIdProperty)] = this.$auth.getClientId(this.name)
    }

    // Only add grant type to payload if enabled
    if (grantTypeProperty !== false) {
      endpoint.data[getRealName(grantTypeProperty)] = grantType
    }
  ...
  async logout (endpoint = {}) {
    ...
    if (this.options.endpoints.logout) {
      ...
      const refreshToken = getRealName(this.options.endpoints.refresh.propertyName)
      if (refreshToken) {
        if (!endpoint.data) {
          endpoint.data = {}
        }
