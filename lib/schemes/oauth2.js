...
import { encodeQuery, normalizePath, parseQuery } from '../utilities'
...
export default class Oauth2Scheme {
  ...
  get _redirectURI() {
    ...
    if (process.server && this.req) {
      ...
      return protocol + this.req.headers.host + normalizePath(this.$auth.ctx, this.$auth.options.redirect.callback)
    }

    if (process.client) {
      return window.location.origin + normalizePath(this.$auth.ctx, this.$auth.options.redirect.callback)
    }
  }
  ...
  async _handleCallback(uri) {
    ...
    if (this.$auth.options.redirect && normalizePath(this.$auth.ctx, this.$auth.ctx.route.path) !== normalizePath(this.$auth.ctx, this.$auth.options.redirect.callback)) {
      return
    }
