...
export default class Oauth2Scheme {
  ...
  get _redirectURI() {
    ...
    if (process.server && this.req) {
      ...
      return protocol + this.req.headers.host + this.$auth.ctx.base + this.$auth.options.redirect.callback
    }

    if (process.client) {
      return window.location.origin + this.$auth.ctx.base + this.$auth.options.redirect.callback
    }
  }
