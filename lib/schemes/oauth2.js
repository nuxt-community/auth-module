...
export default class Oauth2Scheme {
  ...
  login () {
    ...
    this.$auth.$storage.setStorage(this.name + '.state', opts.state)
  ...
  async _handleCallback (uri) {
    ...
    // Validate state
    const state = this.$auth.$storage.getStorage(this.name + '.state')
    this.$auth.$storage.setStorage(this.name + '.state', null)
