import { encodeQuery, ExpiredAuthSessionError, parseQuery, RefreshController } from '../utilities'
...
export default class Oauth2Scheme {
  ...
  initializeTokenRefreshOnRequest () {
    ...
    $axios.onRequest(async config => {
      ...
      // If already trying to refresh token, wait for it to complete
      ...
      try {
        tokenExpiresAt = jwtDecode(token).exp * 1000
        refreshTokenExpiresAt = jwtDecode(refreshToken).exp * 1000
      } catch (e) {
      ...
      if (now > refreshTokenExpiresAt) {
        ...
        // Stop the request from happening. The original caller must catch ExpiredAuthSessionErrors
  ...
  async refreshTokens() {
    ...
    const response = await $axios.post(this.options.access_token_endpoint,
      encodeQuery({
    ...
    const newToken = this.$auth.addTokenPrefix(response.data.access_token)
    const newRefreshToken = this.$auth.addTokenPrefix(response.data.refresh_token)
