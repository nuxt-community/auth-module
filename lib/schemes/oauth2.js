...
export default class Oauth2Scheme {
  ...
  get _redirectURI () {
    ...
    if (process.server && this.req) {
      ...
      return protocol + this.req.headers.host + normalizePath(this.$auth.options.redirect.callback) // don't pass in context since we want the base path
    }

    if (process.client) {
      return window.location.origin + normalizePath(this.$auth.options.redirect.callback) // don't pass in context since we want the base path
    }
  }
