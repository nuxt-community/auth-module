...
export default class Oauth2Scheme {
  ...
  async mounted () {
    ...
    // necessary for each SSR request
    if (!redirected) {
  ...
  _handleLogoutRedirect (logoutPath, idTokenHint) {
    const logoutRequestParams = {
      id_token_hint: idTokenHint,
      post_logout_redirect_uri: window.location.origin + logoutPath
    }

    const redirectLocation = this.options.logout_endpoint + '?' + encodeQuery(logoutRequestParams)
  ...
  async logout () {
    ...
    const logoutRedirectAddress = this.$auth.options.redirect.logout
    const idTokenHint = this.$auth.$storage.getUniversal('id_token')

    if (logoutRedirectAddress && idTokenHint) {
      this._handleLogoutRedirect(logoutRedirectAddress, idTokenHint)
    }
  ...
  async _handleCallback (uri) {
    ...
    if (parsedQuery.id_token) {
      this.$auth.$storage.setUniversal('id_token', parsedQuery.id_token)
    }
